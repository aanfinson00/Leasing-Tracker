// Parent-agnostic contacts panel. Used by both DevelopmentProjectDrawer
// (links = DevProjectContact[]) and AcquisitionTargetDrawer (links =
// AcquisitionTargetContact[]). The component knows nothing about the
// parent's ID field name — the caller builds the full link and passes
// it back through onLink.
...
/** Generic contact-link shape — every parent type's link conforms. */
export interface CrmContactLink {
  id: string;
  contactId: string;
  roleOverride: ContactType | null;
  isPrimary: boolean;
...
interface CrmContactsPanelProps {
  ...
  /** Caller assembles the full parent-specific link (with their parent
   *  FK field) from the (contactId, isPrimary) pair. */
  onLink: (contactId: string, isPrimary: boolean) => void;
...
export function CrmContactsPanel({
  ...
  const handlePickExisting = (contactId: string) => {
    ...
    if (!contact) return;
    onLink(contactId, links.length === 0);
    setAdding(false);
  };
  ...
  const handleSaveNew = (contact: Contact) => {
    ...
    onLink(contact.id, links.length === 0);
    setCreating(null);
  };
  ...
  return (
    <div className="rounded-lg bg-bg shadow-soft p-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <input value={companyName} onChange={(e) => setCompanyName(e.target.value)} placeholder="Company" className={fieldClass} />
        <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Title (optional)" className={fieldClass} />
        <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone" type="tel" className={`${fieldClass} tabular-nums`} />
        <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email" type="email" className={fieldClass} />
      </div>
      <div className="flex items-center justify-end gap-2 mt-2.5">
        <button type="button" onClick={onCancel} className="px-2.5 py-1 text-xs font-medium text-fg-muted hover:text-fg rounded transition-colors">
          Cancel
        </button>
        <button type="button" onClick={handleSubmit} className="inline-flex items-center gap-1 px-3 py-1 text-xs font-semibold text-accent-fg bg-accent rounded-lg hover:bg-accent-hover transition-colors shadow-soft">
