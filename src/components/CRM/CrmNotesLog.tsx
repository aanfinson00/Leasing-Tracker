// Parent-agnostic activity log. Used by both DevelopmentProjectDrawer
// and AcquisitionTargetDrawer. The component knows nothing about
// the parent's FK field — the caller's onSave receives the
// composed note draft and assembles the full record with parent ID.

import { useState } from 'react';
import { Plus, X, Trash2, Phone, NotebookPen } from 'lucide-react';
import type { DevNoteType } from '../../types';
import { DevNoteTypeEnum } from '../../types';

/** Generic note shape — every parent type's note conforms. */
export interface CrmNote {
  id: string;
  noteType: DevNoteType;
...
interface CrmNotesLogProps {
  notes: CrmNote[];
  onCreate: (draft: CrmNoteDraft) => void;
...
export function CrmNotesLog({ notes, onCreate, onDelete }: CrmNotesLogProps) {
    ...
    <div className="flex flex-col gap-2">
      ...
      {sorted.length === 0 && !adding ? (
        ...
        <ul className="flex flex-col gap-2">
          {sorted.map((n) => (
            <li key={n.id} className="rounded-lg bg-bg shadow-soft p-3 flex gap-3">
              <div
