...
// Phase 1 cash-flow schedule. Wide horizontal table — sticky first
// column, dates running left → right. Cleaner port of Lease-
// Calculator's CashFlowSchedule with the comparison row stripped.

export function CashFlowSchedule({ inputs, results }: Props) {
      ...
      {open && (
        <div className="mt-4 flex flex-col gap-2">
          <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1 text-xs text-fg-muted">
            <span>Execution {inputs.leaseExecutionDate}</span>
            <span>·</span>
            <span>Commencement {inputs.leaseCommencement}</span>
            {free > 0 && (
              <>
                <span>·</span>
                <span>Rent comm. M{rcMonth}</span>
              </>
            )}
            <span>·</span>
            <span>all values $/SF</span>
          </div>
